...
// Check returns a boolean policy decision.
func (opa *OPA) Check(ctx context.Context, input interface{}, opts ...func(*rego.Rego)) (bool, error) {
	...
	err := storage.Txn(ctx, opa.manager.Store, storage.TransactionParams{}, func(txn storage.Transaction) error {
		...
		} else if _map, ok := rs[0].Expressions[0].Value.(map[string]interface{}); !ok || len(rs) > 1 {
			return fmt.Errorf("non-map decision")
		} else {
			if b, ok := _map["allow"].(bool); !ok {
